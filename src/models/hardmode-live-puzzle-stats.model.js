...
const CREATE_TABLE = `
  CREATE TABLE IF NOT EXISTS ipl_hardmode_live_puzzle_stats (
...
async function createTable() {
  ...
  try {
    await pool.execute("ALTER TABLE ipl_hardmode_live_puzzle_stats ADD COLUMN game_starts INT NOT NULL DEFAULT 0");
  } catch { /* column already exists */ }
}

async function findByDay(puzzleDay) {
  const [rows] = await pool.execute(
    "SELECT * FROM ipl_hardmode_live_puzzle_stats WHERE puzzle_day = ?",
    [puzzleDay]
  );
  return rows[0] || null;
}
...
async function increment(puzzleDay, won, numGuesses) {
  await pool.execute(
    `INSERT INTO ipl_hardmode_live_puzzle_stats (puzzle_day, total_played, total_won,
...
async function findLatestDay() {
  const [rows] = await pool.execute(
    `SELECT hlps.* FROM ipl_hardmode_live_puzzle_stats hlps
     INNER JOIN ipl_hardmode_daily_puzzles ihdp ON ihdp.day = hlps.puzzle_day
     WHERE ihdp.set_at < CURDATE() + INTERVAL 1 DAY
     ORDER BY ihdp.day DESC
     LIMIT 1`
  );
  return rows[0] || null;
}
...
async function incrementGameStart(puzzleDay) {
  await pool.execute(
    `INSERT INTO ipl_hardmode_live_puzzle_stats (puzzle_day, game_starts)
     VALUES (?, 1)
