...
const CREATE_TABLE = `
  CREATE TABLE IF NOT EXISTS random_challenge_guesses (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    room_id         INT           NOT NULL,
    round_id        INT           NULL,
    player_role     ENUM('player_a','player_b') NOT NULL,
...
async function create(entry) {
  ...
  const [result] = await pool.execute(
    `INSERT INTO random_challenge_guesses
...
async function findByRoom(roomId) {
  const [rows] = await pool.execute(
    "SELECT * FROM random_challenge_guesses WHERE room_id = ? ORDER BY guessed_at ASC",
    [roomId]
  );
  return rows;
}
...
async function findByRound(roundId) {
  const [rows] = await pool.execute(
    "SELECT * FROM random_challenge_guesses WHERE round_id = ? ORDER BY guessed_at ASC",
    [roundId]
  );
  return rows;
}
...
async function findByRoundAndRole(roundId, role) {
  const [rows] = await pool.execute(
    "SELECT * FROM random_challenge_guesses WHERE round_id = ? AND player_role = ? ORDER BY guess_number ASC",
    [roundId, role]
  );
  return rows;
}
...
async function findByRoomAndRole(roomId, role) {
  const [rows] = await pool.execute(
    "SELECT * FROM random_challenge_guesses WHERE room_id = ? AND player_role = ? ORDER BY guess_number ASC",
    [roomId, role]
  );
  return rows;
}
...
async function getGuessCount(roomId, role) {
  const [[{ cnt }]] = await pool.execute(
    "SELECT COUNT(*) AS cnt FROM random_challenge_guesses WHERE room_id = ? AND player_role = ?",
    [roomId, role]
  );
  return cnt;
}
