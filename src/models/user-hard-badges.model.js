...
const CREATE_TABLE = `
  CREATE TABLE IF NOT EXISTS user_hard_badges (
...
async function findByUserId(userId) {
  const [rows] = await pool.execute(
    "SELECT * FROM user_hard_badges WHERE user_id = ?",
    [userId]
  );
  return rows[0] || null;
}
...
async function ensureRow(userId) {
  await pool.execute(
    "INSERT IGNORE INTO user_hard_badges (user_id) VALUES (?)",
    [userId]
  );
}
...
async function applyAfterHardGame(userId, { maxStreak, won, numGuesses }) {
